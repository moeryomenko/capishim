...
# capishim-core -- Cluster API core controller manager built from the pinned
# upstream cluster-api tag (REQ-011). The manager binary is compiled from the
# upstream source at CAPI_SOURCE_REF; the same tag is recorded in the final
# image label so built-from provenance is inspectable (REQ-011 PASS).
...
RUN --mount=type=cache,id=capishim-gomod-${CAPI_SOURCE_REF},target=/go/pkg/mod \
    ...
    -ldflags "-extldflags '-static'" -o /manager ./core
