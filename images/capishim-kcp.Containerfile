...
# capishim-kcp -- kubeadm control plane (KCP) controller manager built from the
# pinned upstream cluster-api tag (REQ-011). Built from
# controlplane/kubeadm/main.go; the pinned tag is recorded in the final image
...
RUN --mount=type=cache,id=capishim-gomod-${CAPI_SOURCE_REF},target=/go/pkg/mod \
    ...
    -ldflags "-extldflags '-static'" -o /manager ./controlplane/kubeadm
